...
/**
 * Igual que BaseMenuContainer de 3×9, pero para menús de 6 filas (54 slots):
 *
 ...
 *      restaura simultáneamente:
 *        – TODOS los slots del menú,
 *        – TODOS los slots del inventario del jugador,
 *        – limpia el cursor,
 *        – reenvía broadcastChanges() de menú e inventario.
 *
 * Ahora, en lugar de usar GENERIC_9x3/3 filas, usamos GENERIC_9x6/6 filas.
 */
public abstract class BaseMenuContainer6 extends ChestContainer {

    private final Inventory menuInv;
    private final int menuSize;

    public BaseMenuContainer6(int windowId,
                              net.minecraft.entity.player.PlayerInventory playerInv,
                              Inventory menuInventory) {
        // Atención: aquí se indica GENERIC_9x6 y 6 filas
        super(ContainerType.GENERIC_9x6, windowId, playerInv, menuInventory, 6);
        this.menuInv = menuInventory;
        this.menuSize = menuInv.getContainerSize(); // normalmente 54
        this.menuInv.startOpen(playerInv.player);

        // Reemplazamos los primeros menuSize slots por MenuSlot
     ...
     *         – clear cursor
     *         – handleButtonClick(...)
     *         – Si seguimos en este contenedor, restaurar el slot y broadcast.
     *    b) Cualquier otro clic en MENÚ:
     *         – Restaurar TODOS los slots del menú + inventario + broadcast.
     * 2) Si clic en slot de inventario del jugador:
     *         – Restaurar TODOS los slots del menú + inventario + broadcast.
     * 3) Si slotId fuera de rango (<0 o ≥ slots.size()), delegar a super.clicked().
     */
    @Override
    public ItemStack clicked(int slotId, int dragType, ClickType clickTypeIn, PlayerEntity player) {
        ...
        if (isMenuSlot(slotId)) {
            ...
            if (player instanceof ServerPlayerEntity) {
                ...
                // Si seguimos en este mismo contenedor, restauramos TODO y broadcast
                if (sp.containerMenu == this) {
                    restoreAllMenuSlots(sp);
                    restoreAllPlayerInventorySlots(sp);
                    clearPlayerCursor(sp);
        ...
        if (isPlayerInvSlot(slotId)) {
            if (player instanceof ServerPlayerEntity) {
                ...
                // Si seguimos en este mismos contenedor, restaurar TODO + broadcast
                if (sp.containerMenu == this) {
                    restoreAllMenuSlots(sp);
                    restoreAllPlayerInventorySlots(sp);
                    clearPlayerCursor(sp);
    ...
    /**
     * Cada subclase (EveryLevelMenuContainer, ExactLevelMenuContainer, PackagesLevelMenuContainer, etc.)
     * IMPLEMENTA este método para definir la acción de cada botón. Si esa acción
     * cierra el contenedor actual (player.closeContainer()), la comprobación
     * “sp.containerMenu == this” evitará que no intente restaurar los slots de un contenedor ya cerrado.
     */
