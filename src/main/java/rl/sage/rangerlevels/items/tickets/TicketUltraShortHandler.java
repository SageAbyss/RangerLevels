...
/**
 * Listener para “Ticket Ultra Rápido” (ID = "ticket_ultra_short"):
 *  1) Comprueba que el ItemStack tenga NBT “RangerID” = "ticket_ultra_short".
 *  2) Restaura el pase anterior si expiró y notifica.
 *  3) Si no tiene Ultra, lo otorga por 10 segundos; si ya tiene, muestra tiempo restante.
 *  4) Cancela cualquier RightClickBlock si el ID coincide.
 */
@Mod.EventBusSubscriber(modid = RangerLevels.MODID)
public class TicketUltraShortHandler {

    private static final long ULTRA_SHORT_DURATION_MS = Duration.ofSeconds(10).toMillis();
    private static final String ID_TICKET_ULTRA_SHORT = "ticket_ultra_short";
    ...
    public static void onRightClickItem(PlayerInteractEvent.RightClickItem event) {
        ...
        if (!ID_TICKET_ULTRA_SHORT.equals(id)) return;
        ...
        // 3) Si ya tiene Ultra (o Master), mostrar tiempo restante
        if (actual.getTier() >= PassType.ULTRA.getTier()) {
            ...
            if (expiresAt != Long.MAX_VALUE) {
                ...
                if (millisLeft > 0) {
                    ...
                    long seconds = duration.toHours() * 3600 + duration.minusHours(duration.toHours()).toMinutes() * 60 + duration.minusHours(duration.toHours()).minusMinutes(duration.minusHours(duration.toHours()).toMinutes()).getSeconds();
        ...
        // 4) Otorgar Ultra Pass (10 segundos), guardando el pase anterior
        IPassCapability cap = PassCapabilities.get(serverPlayer);
        cap.grantPass(PassType.ULTRA.getTier(), ULTRA_SHORT_DURATION_MS);
        ...
        IFormattableTextComponent linea1 = new StringTextComponent(TextFormatting.GREEN + "❖ ᴀᴄᴛɪᴠᴀᴅᴏ ")
                ...
                .append(new StringTextComponent(TextFormatting.GREEN + " por 10 segundos!"));
    ...
    public static void onRightClickBlock(PlayerInteractEvent.RightClickBlock event) {
        ...
        String id = RangerItemDefinition.getIdFromStack(held);
        if (ID_TICKET_ULTRA_SHORT.equals(id)) {
            event.setCanceled(true);
            event.setCancellationResult(ActionResultType.SUCCESS);
        }
    }
