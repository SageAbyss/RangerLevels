// File: rl/sage/rangerlevels/items/sacrificios/ModificadorIVsUniversalHandler.java
...
public class ModificadorIVsUniversalHandler {
    ...
    public static void onRightClickItem(PlayerInteractEvent.RightClickItem ev) {
        ...
        if (held.isEmpty() ||
                !ModificadorIVsUniversal.ID.equals(RangerItemDefinition.getIdFromStack(held)))
            return;

        // Si falta acción o stat, abrimos el menú universal
        if (ModificadorIVsUniversal.getStat(held) == null) {
            ev.setCanceled(true);
    ...
    public static void onEntityRightClick(PlayerInteractEvent.EntityInteractSpecific ev) {
        ...
        String action = ModificadorIVsUniversal.getAction(held);
        String stat   = ModificadorIVsUniversal.getStat(held);
        ...
        // 2) Que sea legendario o Ultraente
        ...
        // 3) Aplicar ajuste de IV
        int value = action.equals("up") ? 31 : 0;

        // Mapeo explícito de stat a BattleStatsType
        BattleStatsType statType;
        switch (stat) {
            case "HP":           statType = BattleStatsType.HP; break;
            case "ATTACK":       statType = BattleStatsType.ATTACK; break;
            case "DEFENSE":      statType = BattleStatsType.DEFENSE; break;
            case "SP_ATTACK":    statType = BattleStatsType.SPECIAL_ATTACK; break;
            case "SP_DEFENSE":   statType = BattleStatsType.SPECIAL_DEFENSE; break;
            case "SPEED":        statType = BattleStatsType.SPEED; break;
            default:
        ...
        // Consumir el modificador y feedback
        held.shrink(1);
